...
class Graph(commands.Cog):
    ...
    async def graph(self, ctx: commands.Context, *, crop: str):
        ...
        if crop_list[0]['ratio'] >= 0.5 and crop_list[0]['ratio'] > crop_list[1]['ratio']*1.02 or crop_list[0]['ratio'] >= crop_list[1]['ratio']*1.4: # ratio가 50% 이상이고 다음 ratio에 비해 2% 이상 높거나 / 가장 높은 ratio가 두 번째로 높은 ratio보다 40% 이상 높을 경우
            ...
            if response_code == 404: await ctx.reply(f"{result['icon']} **{result['name']}**는 최근 1주일간 거래된 기록이 없습니다."); return
            ...
            await ctx.reply(f"{result['icon']} **{result['name']}**의 최근 1주일 시세 그래프입니다.", file=discord.File("trade.png"))
            ...
            for i in range(len(crop_list)):
                if crop_list[i]['ratio'] == 0 or crop_list[0]['ratio'] >= crop_list[i]['ratio']*1.4: # ratio가 0이거나 가장 높은 ratio에 비해 40% 이상 낮은 경우
                    break
                description += f"{crop_list[i]['icon']} **{crop_list[i]['name']}**\n"
